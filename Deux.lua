...
Shop = {Grounds = {['Web'] = {100, 15, P = {0.3, 0.2}}, ['Wather'] = {100, 9, P = {0.3, 0.2}}, ['Wood'] = {0, 0, P = {0.3, 0.2}}, ['Sand'] = {75, 7, P = {0.1, 0.2}}, ['Earth'] = {100, 5, P = {0.3, 0.2}}, ['Grass'] = {150, 6, P = {0.3, 0.2}}, ['Stone'] = {500, 10, P = {0.3, 0}}, ['Colorful'] = {750, 12, P = {0.3, 0.2}}, ['Chocolate'] = {1000, 4, P = {20, 0.2}}}, Skills = {['Resize'] = {200, 200}}}
PDB = {}
...
eventShop = function(P, C)
	local T = '<br><br><n>Grounds:'
	...
	local K = '<br><br><n>Skills:'
	for S in pairs(Shop.Skills) do
		if PDB[P].Inventory[S] and not PDB[P].Skills[S] then
			K = K .. '<br><bl>» ' .. S .. ' : <v><a href="event:Equip_' .. S .. '">Equip</a>'
		elseif PDB[P].Skills[S] then
			K = K .. '<br><bl>» ' .. S .. ' : <v><a href="event:Unequip_' .. S .. '">Unequip</a>'
		elseif not PDB[P].Inventory[S] then
			K = K .. '<br><bl>» ' .. S .. ' : <v><a href="event:Buy_' .. S .. '">Buy ' .. Shop.Skills[S][1] .. '</a>'
		end
	end
	if C == 1 then
		return T
...
eventChatCommand = function(P, C)
	...
	if C == 'p' and not A[2] and not PDB[P].O then
		...
		ui.addTextArea(3, '<a href="event:pc"><p align="center"><font size="12">Close</font></p></a>', P, 295, 278, 210, nil, 0x27373F, 0x324650, .8, true)
	...
	elseif C == 's' and not PDB[P].O then
		PDB[P].O = true
		ui.addTextArea(4, '<a href="event:grounds"><font size="12">Grounds</font></a>', P, 220, 110, 105, nil, 0x324650, 0x27373F, 1, true)
		ui.addTextArea(5, '<a href="event:skills"><font size="12">Skills</font></a>', P, 220, 140, 105, nil, 0x324650, 0x27373F, 1, true)
		ui.addTextArea(6, string.format('<font size="20">Shop</font>%s', eventShop(P, 1)), P, 290, 100, 220, 202, 0x324650, 0x27373F, 1, true)
		ui.addTextArea(7, '<a href="event:sc"><p align="center"><font size="12">Close</font></p></a>', P, 295, 278, 210, nil, 0x27373F, 0x324650, .8, true)
...
eventTextAreaCallback = function(I, P, C)
	...
	elseif C == 'grounds' then
		ui.updateTextArea(6, string.format('<n><font size="20">Shop</font>%s', eventShop(P, 1)), P)
	elseif C == 'skills' then
		ui.updateTextArea(6, string.format('<n><font size="20">Shop</font>%s', eventShop(P, 2)), P)
	end
end
...
eventSummoningEnd = function(p, o, x, y, a, xs, ys, t, e)
	if o == 0 then
		if PDB[p].HaM < 1 then
			...
			tfm.exec.addPhysicObject(Ground, x, y, {type = PDB[p].GroundU, width = 100, height = 10, angle = a, friction = Shop.Grounds[PDB[p].GroundN].P[1], restitution = Shop.Grounds[PDB[p].GroundN].P[2], color = PDB[p].Colorful})
		elseif PDB[p].HaM > 0 then
			tfm.exec.addPhysicObject(1, x, y, {type = PDB[p].GroundU, width = 100, height = 10, angle = a, friction = Shop.Grounds[PDB[p].GroundN].P[1], restitution = Shop.Grounds[PDB[p].GroundN].P[2], color = PDB[p].Colorful})
		else
			print(error())
		end
	elseif o == 24 then
		...
		if Sps > 2 then
			...
			tfm.exec.chatMessage('<J>Alert! You used a spirit. Remainder: <v>' .. 2 - Sps, p);
		end
